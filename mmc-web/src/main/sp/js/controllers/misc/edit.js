...
angular.module('mmcApp').controller('miscEditResultCtrl', function($scope, $modalInstance, utils, result) {
 $scope.result = result;
 utils.debug('miscEditResultCtrl doc: ' + JSON.stringify(result));
...
.controller('miscEditCtrl', ['$document', '$scope', '$rootScope', '$http', '$location', '$routeParams'
...
function($document, $scope, $rootScope, $http, $location, $routeParams
  ...
  modalInstance.result.then(function (event) {
   if ('performDelete' == event) {
	var callback = function() {
	 $location.path('/misc/find');
	};
	musicService.remove($scope.doc.id, 'misc', callback, callback);	   
   } else if ('back' == event) {
    $location.path('/misc/view/' + $scope.result.id);  
   }
   utils.debug(event);	  
  });
 };
 ...
 $scope.edit = function(docId, tabId) {
  ...
  $scope.updatePrice = {};
  $scope.updatePrices = [];
  musicService.getDoc(docId, 'misc', function(response) {
   $scope.doc = response;
 ...
 $scope.update = function() {
  ...
  musicService.updateDoc($scope.doc, 'misc', function(response) {
 ...
 $scope.uploadFile = function(fileItem) { 
  ...
  fileItem.status='p';
  musicService.uploadPhoto(docId, 'misc', fileItem.file, function(photo) {
 ...
 $scope.removePhotos = function() {
  ...
  if (selectedPhotoIds.length > 0) {
   musicService.removePhotos($scope.doc.id, 'misc', selectedPhotoIds, function(doc) {
  ...
  */
 $scope.view = function() {
  $location.path('/misc/view/' + $scope.doc.id);
 };
 ...
 $scope.uploadImages = function() {
  ...
  var uri = '/misc/edit/' + $scope.doc.id;	 
  $location.path(uri); 
 };

 $scope.edit($routeParams.miscDocId, $routeParams.tabId);
