...
angular.module('mmcApp').controller('miscAddResultCtrl', function($scope, $modalInstance, utils, result) {
 $scope.result = result;
 utils.debug('miscAddResultCtrl doc: ' + JSON.stringify(result));
...
.controller('miscAddCtrl', ['$scope', '$http', '$location', 'musicService','userService', 'utils', 'refValues', '$modal',
function($scope, $http, $location, musicService, userService, utils, refValues, $modal) {
  ...
  modalInstance.result.then(function (mode) {
   ...
   if (mode == 'newDoc') {
	$scope.init();
	 ...
	 $location.path('/misc/edit/' + mode.split('#')[1] + '/photos');  
   }
  }, function () {
 ...
 $scope.submit = function() {
   musicService.addDoc($scope.doc, 'misc', function(response) {
   $scope.doc = response; 
