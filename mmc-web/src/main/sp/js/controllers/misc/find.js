...
.controller('miscListCtrl', ['$scope', 'musicService', 'userService', 'refValues', 'appService', 'utils', 
function ($scope, musicService, userService, refValues, appService, utils) {

 $scope.$on('misc', function(event, args) {
  $scope.list(0, 'misc');	 
 });
 ...
 musicService.clearCache('misc');
 $scope.list(0, 'misc');
 
}]);


angular.module('mmcApp').controller('miscListingCtrl', ['$scope', 'appService', 'musicService', '$sce', 
function ($scope, appService, musicService, $sce) {
 $scope.wating = true;
 $scope.listing=true;
 musicService.getDocs(appService.app().query, -1, false, 'misc', 'pdf', function(response) {
